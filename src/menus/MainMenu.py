...
class MainMenu:
    def __init__(self, game):
        ...
        # Création du bouton Charger Sauvegarde
    ...
    def process_event(self, evt):
        if evt.type == pygame.MOUSEBUTTONDOWN and evt.button == pygame.BUTTON_LEFT:
            if self.quit_button.get_rect(x=500, y=725).collidepoint(*evt.pos): # Appui sur le bouton Quitter
                self.game.stop()
            elif self.play_button.get_rect(x=500, y=325).collidepoint(*evt.pos): # Appui sur le bouton Jouer
                self.game.display(1)
            elif self.load_button.get_rect(x=500, y=525).collidepoint(*evt.pos): # Appui sur le bouton Charger Sauvegarde
                self.game.display(2)