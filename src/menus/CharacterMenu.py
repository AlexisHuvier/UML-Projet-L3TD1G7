...
class CharacterMenu:
    ...
    def process_event(self, evt):
        if evt.type == pygame.MOUSEBUTTONDOWN and evt.button == pygame.BUTTON_LEFT:
            if self.standart_button.get_rect(x=500, y=325).collidepoint(*evt.pos):
                self.game.create_player(0)
                self.game.display(5)
            elif self.hippie_button.get_rect(x=500, y=525).collidepoint(*evt.pos):
                self.game.create_player(1)
                self.game.display(5)
            elif self.presse_button.get_rect(x=500, y=725).collidepoint(*evt.pos):
